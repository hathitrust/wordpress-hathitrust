(function() {
	...
	function setSidebarNavigationHoverSpecs(el) {
		...
		if (hoverWidth === 0 && hoverHeight === 0) {
			...
			navmenu.style.setProperty('--left', `${el.parentElement.getBoundingClientRect().left + window.pageXOffset}px`);
			navmenu.style.setProperty('--top', `${el.parentElement.getBoundingClientRect().top + window.pageYOffset}px`);
	...
	function setupSidebarNavigationMenu() {
		...
		submenus.forEach(function(el, i) {
			...
			submenuToggler.innerHTML = '<svg width="9" height="5" aria-hidden="true"><path d="m8.266 1.64-2.977 3a.826.826 0 0 1-.539.21.742.742 0 0 1-.54-.21l-2.976-3a.726.726 0 0 1-.187-.82.768.768 0 0 1 .703-.47h5.977c.304 0 .562.188.68.47.116.28.07.609-.141.82Z" fill="currentColor"></svg>';
	...
	function setupSectionJumper() {
		...
		document.querySelectorAll('.twocol-main h2').forEach(function(el) {
			...
			slug = el.textContent.replace(/ /g, '-').replace(/[^A-Za-z0-9-]/g, '').toLowerCase();
	...
	function checkTableOverflow(table) {
		...
		if (tableWrapper.scrollWidth > tableWrapper.parentElement.offsetWidth || tableWrapper.scrollWidth > document.body.offsetWidth) {
